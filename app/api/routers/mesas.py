...
def crear_mesa(payload: MesaCreate, session: Session = Depends(get_session), current_user=Depends(require_roles("propietario"))):
...
def actualizar_mesa(idmesa: str, payload: MesaCreate, session: Session = Depends(get_session), current_user=Depends(require_roles("propietario"))):
    return MesasService(session).actualizar(idmesa, payload)

@router.delete("/{idmesa}", status_code=status.HTTP_204_NO_CONTENT)
def eliminar_mesa(idmesa: str, session: Session = Depends(get_session), current_user=Depends(require_roles("propietario"))):
